...
class TranscriptProcessor:
    ...
    def format_transcript_text(text: str, transcript_json: Optional[Dict] = None) -> str:
        ...
        if transcript_json and isinstance(transcript_json, dict):
            ...
            # Paragraphs are nested inside the transcript object, not at top level!
            transcript_obj = transcript_json.get('transcript', {})
            paragraphs = transcript_obj.get('paragraphs', []) if isinstance(transcript_obj, dict) else []
            ...
            elif speaker_mapping and (not paragraphs or len(paragraphs) == 0):
                # Has speaker_mapping but no paragraphs - log for diagnostics but not necessarily an error
                logger.info(f"V3 transcript has speaker_mapping ({len(speaker_mapping)} speakers) but no paragraphs array. Using plain text formatting.")
